    ...
    public boolean function setFormFields(
        ...
        try {
            ...
            if (structKeyExists(arguments, "fdfdata") && arguments.fdfdata != "") {
                ...
                local.fdf = local.Loader.loadFDF(local.fileInputStream);
                ...
                local.stFields = local.pdfForm.getFieldIterator();
                writeDump(  local.stFields);
                writeDump(  local.stFields.next());
                writeDump(  local.stFields.next().getAcroForm());
                writeDump(  local.stFields.next().getAcroForm().getFields()[2].getValueAsString());
                while (local.stFields.hasNext()) {
                    var fieldName = local.stFields.next();
